...
from db import get_gifts_by_price, get_gift_details
...
def gift_menu(call, bot: TeleBot):
    ...
    bot.edit_message_text("Выберите подарок: (Уведомите нас при выборе подарка по номеру: +7-(777)-880-3100)", call.message.chat.id, call.message.id, reply_markup=markup)
...
def buy_full(call, bot: TeleBot):
    ...
    if not gift:
        ...
        return
    name, price, description, link = gift
    # Удалено удаление из базы данных: delete_gift(gift_id)
...
def final_selection(call, bot: TeleBot):
    bot.edit_message_text("🎁 Спасибо! Ваш подарок выбран.",
                          call.message.chat.id, call.message.id)
